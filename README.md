...
#### This script can rename the PC, join a domain, remove OneDrive, remove all Desktop, Startmenu, Taskbar icons (Including the People, Taskview, and Action Center icons on the Taskbar) delete all Windows Store apps (except the Calculator and Photos and the Windows Store) and install Chocolatey with selected packages and major system dependencies. (Dependencies include all .NET Framework and VCRedist versions)
```powershell
